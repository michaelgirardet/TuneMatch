...
export default function AddTrackModal({ isOpen, onClose, onAdd }: AddTrackModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!videoId) {
      ToasterError('🎬 Oups ! Cette vidéo YouTube semble invalide.');
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ToasterError('❌ Vidéo introuvable ou restreinte. Vérifie l’URL !');
        return;
      }
    } catch {
      ToasterError('🔍 Impossible de vérifier cette vidéo. Essaie une autre.');
      return;
    }
