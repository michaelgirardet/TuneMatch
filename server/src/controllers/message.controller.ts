...
export const getConversations: AuthRequestHandler = async (req, res) => {
  try {
...
export const getMessages: AuthRequestHandler = async (req, res) => {
  try {
    const userId = req.user?.userId;
...
// Envoyer un message
export const sendMessage: AuthRequestHandler = async (req, res) => {
  try {
    const userId = req.user?.userId;
    if (!userId) {
      return res.status(401).json({ error: 'Utilisateur non authentifié' });
    ...
    res.status(201).json({
      ...
      messageId: (result as { insertId: number }).insertId,
    });
  } catch (error) {
